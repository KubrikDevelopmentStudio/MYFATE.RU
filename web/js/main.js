...
$(document).ready(function(){
    ...
    setInterval(function() {
        $('.aMore').on('mouseout', function() {
            $('.other').animate({'margin-top':'200px', 'opacity':'0'}, {queue:false, duration:500});
        })
    }, 500);

    $('.reg').on('click', function() {
        $('.main-signin').animate({'margin-left':'-1000px'}, {queue:false, duration:500});
        $('.main-signin1').animate({'margin':'-610px 257px', 'left': '475px'}, {queue:false, duration:500}).css({'display':'block'});
    });
    setInterval(function() {
        $('.revertToAut').on('click', function() {
            $('.main-signin').animate({'margin-left':'725px'}, {queue:false, duration:500});
            $('.main-signin1').animate({'margin':'-610px 3000px', 'left':'-467px'}, {queue:false, duration:500});
        })
    }, 500);

    $('.revertToAut').on('mouseout', function() {
        $('.revertToAut').stop(true, false);
        $('.revertToAut').animate({'margin-left':'190px'}, 300);
    })
    $('.revertToAut').on('mouseover', function() {
